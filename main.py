...
from remote_tools import XRayVQATool, XRayPhraseGroundingTool
...
def initialize_agent(
    ...
    model = ChatOpenAI(
        model="Gemma-3-4b-it",   # 和你 vLLM serve 的 --served-model-name 一致
        temperature=0.2,                # 和 quickstart 保持一致
        top_p=0.95,
        # base_url 可以不写，langchain 会读 OPENAI_BASE_URL
        # api_key 也从 OPENAI_API_KEY 环境变量读
    )
    agent = Agent(
        model,
